...
class BlogController extends Controller
{
    ...
    public function view($blog_slug, $slug)
    {
        ...
        $blog_category_ids = array_merge(explode(',', $blog->default_category), explode(',', $blog->categories));
            ...
            $related_blogs = $related_blogs->whereIn('default_category', $blog_category_ids)->where(
                function ($query) use ($blog_category_ids) {
                    foreach ($blog_category_ids as $key => $blog_category_id) {
                        if ($key == 0) {
                            $query->whereRaw('FIND_IN_SET(?, categories)', [$blog_category_id]);
                        } else {
                            $query->orWhereRaw('FIND_IN_SET(?, categories)', [$blog_category_id]);
                        }
                    }
                }
            );
